...
public class JwTAuthenticationFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		try {
			// HTTP 요청의 Authorization 헤더 JWT를 추출한다.
			...
			if(token != null && !token.equalsIgnoreCase("null")) {
				...
				// 검증에 성공을 했고 스프링 시큐리티가 이해할 수 있는 인증 객체를 만든다.
				// 검증이 끝난 사용자를 "인증 완료된 사용자"로 표현하기 위한것
				AbstractAuthenticationToken authentication = 
						new UsernamePasswordAuthenticationToken(
								userId, // 사용자 정보
								null, // 인증수단(JwT로 인증했으니 null)
				...
				// 비어있는 SecurityContext 객체 생성, 현재 요청의 인증 정보를 저장하는 공간이다.
		...
		// 현재 필터 작업이 끝났으니 요청 처리를 다음 단계로 진행시킨다.
